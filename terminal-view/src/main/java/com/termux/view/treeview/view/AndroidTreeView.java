package com.termux.view.treeview.view;
...
import com.termux.view.treeview.holder.SimpleViewHolder;
...
public class AndroidTreeView {
    ...
    public View getView(int style) {
        ...
        // If using TwoDScrollView, allow owner to attach a scale listener.
        // (Used by file tree to resize rows instead of scaling the whole canvas.)
        if (use2dScroll && view instanceof TwoDScrollView) {
            ((TwoDScrollView) view).setOnScaleChangedListener(new TwoDScrollView.OnScaleChangedListener() {
                @Override
                public void onScale(float scaleFactor) {
                    // No-op by default. Activities can override by setting a custom view holder/static scale.
                    // We keep this hook here so TwoDScrollView can deliver pinch scale events.
                }
        ...
        // Important for TwoDScrollView horizontal panning: allow content to grow wider than viewport.
        if (use2dScroll) {
            viewTreeItems.setLayoutParams(new ViewGroup.LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT, ViewGroup.LayoutParams.WRAP_CONTENT));
        }
    ...
    public void expandNodeAsync(final TreeNode node) {
        expandNodeAsync(node, false);
    }

    public void expandNodeAsync(final TreeNode node, boolean includeSubnodes) {
        node.setExpanded(true);
        final TreeNode.BaseNodeViewHolder parentViewHolder = getViewHolderForNode(node);
        final ViewGroup nodeItemsView = parentViewHolder.getNodeItemsView();
        nodeItemsView.removeAllViews();

        parentViewHolder.toggle(true);

        if (mUseDefaultAnimation) {
            expand(nodeItemsView);
        } else {
            nodeItemsView.setVisibility(View.VISIBLE);
        }
        ...
        // Use a unique token to identify this specific expansion request
        final Object expansionToken = new Object();
        nodeItemsView.setTag(expansionToken);
        ...
        Runnable r = new Runnable() {
            ...
            @Override
            public void run() {
                // If the view's tag has changed (new expansion started) or node collapsed, stop.
                if (!node.isExpanded() || nodeItemsView.getTag() != expansionToken) return;

                int end = Math.min(index + batchSize, children.size());
                for (int i = index; i < end; i++) {
                    TreeNode n = children.get(i);
                    addNode(nodeItemsView, n);
                    if (n.isExpanded()) {
                         expandNode(n, false);
                    }
                }
    ...
    private void addNode(ViewGroup container, final TreeNode n) {
        ...
        // Safety check: remove from existing parent if any
