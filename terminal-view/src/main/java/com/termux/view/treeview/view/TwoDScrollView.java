...
public class TwoDScrollView extends FrameLayout {
    ...
    // Gesture detector (pinch zoom callback can be provided by owner)
    ...
    public void setOnScaleChangedListener(OnScaleChangedListener listener) {
        this.mOnScaleChangedListener = listener;
    }
    ...
    private void initTwoDScrollView() {
        ...
        mScaleGestureDetector = new ScaleGestureDetector(getContext(), new ScaleGestureDetector.SimpleOnScaleGestureListener() {
            ...
            public boolean onScale(ScaleGestureDetector detector) {
                // Delegate scaling to the owner (e.g. file tree row scaling).
                if (mOnScaleChangedListener != null) {
                    mOnScaleChangedListener.onScale(detector.getScaleFactor());
                    return true;
                }
                return false;
            }
            ...
            public boolean onScaleBegin(ScaleGestureDetector detector) {
                if (mOnScaleChangedListener != null) {
                    mOnScaleChangedListener.onScaleBegin();
                }
                return true;
            }
            ...
            public void onScaleEnd(ScaleGestureDetector detector) {
                if (mOnScaleChangedListener != null) {
                    mOnScaleChangedListener.onScaleEnd();
                }
            }
        });
    ...
    private void clampScrollToContent() {
        int maxX = Math.max(0, getContentWidth() - (getWidth() - getPaddingLeft() - getPaddingRight()));
        int maxY = Math.max(0, getContentHeight() - (getHeight() - getPaddingTop() - getPaddingBottom()));
        int clampedX = Math.max(0, Math.min(getScrollX(), maxX));
        int clampedY = Math.max(0, Math.min(getScrollY(), maxY));
    ...
    private boolean canScroll() {
        return (getHeight() < getContentHeight() + getPaddingTop() + getPaddingBottom()) ||
            (getWidth() < getContentWidth() + getPaddingLeft() + getPaddingRight());
    }
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        // Pinch zoom should be handled by this view and should not trigger DrawerLayout edge swipe.
        ...
        // Intercept motion events only if we are in drag mode.
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (deltaX < 0) {
                    ...
                    final int rightEdge = getWidth() - getPaddingRight();
                    final int availableToScroll = getContentWidth() - getScrollX() - rightEdge;
                ...
                if (deltaY < 0) {
                    ...
                    final int bottomEdge = getHeight() - getPaddingBottom();
                    final int availableToScroll = getContentHeight() - getScrollY() - bottomEdge;
                    if (availableToScroll > 0) {
